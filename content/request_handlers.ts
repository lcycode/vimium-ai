...
import { aiOnChunk, aiOnDone, aiOnError, aiOnAck } from "./ai_bar"
...
// AI streaming responses use dedicated indices in the sparse kBgReq range
// (kBgReq.ai_chunk = 50 etc., far from the dense 0..16 handlers above).
requestHandlers[kBgReq.ai_chunk] = (request: BgAIBarSpecialReq[kBgReq.ai_chunk]): void => {
  aiOnChunk(request.i, request.d)
}
requestHandlers[kBgReq.ai_done] = (request: BgAIBarSpecialReq[kBgReq.ai_done]): void => {
  aiOnDone(request.i, request.f)
}
requestHandlers[kBgReq.ai_error] = (request: BgAIBarSpecialReq[kBgReq.ai_error]): void => {
  aiOnError(request.i, request.m)
}
requestHandlers[kBgReq.ai_ack] = (request: BgAIBarSpecialReq[kBgReq.ai_ack]): void => {
  aiOnAck(request.i, request.u, request.k, request.m)
}
